...
files_collected_day1 = 46  # Number of ROIs collected each day
files_collected_day2 = 47
files_collected_day3 = 25
...
def configure_mma_file(file_path):
    ...
    df = df.drop(columns=['Local Scan', 'X', 'Z', ' ', ' .1', ' .2', ' .3', ' .4', ' .5', ' .6', ' .7', ' .8', ' .9',
                          'Scan'])
...
def combine_files(mma_df, sa_file, rsd_file):
    ...
    roi_list = ['ROI'+str(roi) for roi in range(1, 119)]
...
def qc_file_convert(mma_file, sa_file, rsd_file, output_file_path):
    mma_df = configure_mma_file(mma_file)
    combined_df = combine_files(mma_df, sa_file, rsd_file)
    save_file(combined_df, output_file_path)
